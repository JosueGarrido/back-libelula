...
export class CreateUserDto {
    ...
    @ApiProperty({ example: 'admin', description: 'Rol del usuario', enum: ['admin', 'broker', 'cliente'] })
    @IsEnum(['admin', 'broker', 'cliente'], { message: 'Invalid role' })
    role: string;

    @ApiProperty({ example: 'entity123', description: 'ID de la entidad a la que pertenece el usuario' })
    @IsNotEmpty()
    entityId: string;
}
